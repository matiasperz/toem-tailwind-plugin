  ...
  return (
    ...
    >
          ...
          return (
            <Fragment key={key}>
              <li>
                <Link
                  href={href}
                  className={clsx(styles["sidebar-link"], {
      ...
      <div className="mt-auto text-sm border-t max-lg:hidden border-zinc-900">
        <div className="flex justify-between px-3 py-2">
          ...
          <button
            ...
            className={clsx(styles["copy-button"], {
              [styles["copy-button-copied"]]: isCopied,
            })}
          >
        ...
        >
          <div
            className={clsx(styles["install-command-bg"], {
              [styles["install-command-bg-copied"]]: isCopied,
            })}
          />
...
export function Sidebar({ children }: { children: React.ReactNode }) {
  return (
    <Popover.Root>
      <div className="sticky top-0 z-50 flex items-center w-full h-16 pr-4 bg-black border-b lg:hidden md:pr-8 border-zinc-900">
        <Popover.Trigger className="relative flex items-center justify-center h-full border-r group border-zinc-900 aspect-square">
          <div className="absolute w-4 h-px -translate-y-1 bg-white group-data-[state=open]:translate-y-0 group-data-[state=open]:rotate-45 transition-transform" />
          <div className="absolute w-4 h-px translate-y-1 bg-white group-data-[state=open]:translate-y-0 group-data-[state=open]:-rotate-45 transition-transform" />
        </Popover.Trigger>
      ...
      <Popover.Portal>
        <Popover.Content className="w-full lg:hidden z-50">
          <SidebarContent mobile />
        </Popover.Content>
      </Popover.Portal>

      <div className="grid min-h-svh w-full grid-cols-1 lg:grid-cols-[max-content_auto]">
        <SidebarContent className="max-lg:hidden" />
